...
- main保留治理入口；候选分支`codex/public-v1`目前收录3个不带数据的模块和2份纯合成单元测试：基础模块2个、服务费配置核心1个。本轮远端回读后24+49项测试通过；不含完整运行服务、公司资料、原始Git历史或发行包。
- 未在开发机建立公开副本或工作树；未改动内部系统。
- 当前阶段：基础源码及服务费配置核心子步骤完成；完整L2 Spec/Plan仍有待定项。奖金配置、Admin页面及服务端接入尚未完成，不等于公开系统可用；尚未建立首次Admin初始化或安装包。
