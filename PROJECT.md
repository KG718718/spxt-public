...
公开封装已恢复，主控串行执行；以下状态优先于后面的历史断点。

- 用户确认税率由 Admin 填写，不预置；未配置阻止需要税率的新计算，历史快照不变。
- MIT 根许可、元数据及第三方材料检查已落地。Windows 换行兼容反例已修复，许可专项 16 项通过。
- 当前运行代码提交 d58aa9fb5a554dd44e422d4792a71032c334dc25：托管 Windows CI 34565261706 通过 13 组 / 382 项；依赖审计 0 漏洞。含真实首次 Admin HTTP 30 项与税率核心 26 项；HTTP 测试只挂载初始化处理器，不代表完整系统。
- 完整 server、5 页参数与授权接入、真实浏览器、原生组件及完整安装/升级/恢复尚未收口，无应用安装包。
- 正在修正只读核对发现的启动校验与员工删除历史凭据保留契约冲突，按失败优先验证；没有真实数据事故证据。
- main 保持原提交；无 PR、合并、Release、部署、内部代码/数据/服务或正式主机操作。公开文件与证据只保存云端。
