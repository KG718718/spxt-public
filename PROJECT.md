...
**完整运行代码与五页已接入；托管 Windows 整站验收和核心程序封装通过。独立安装器尚未完成，候选未发布。**

- 已验代码：`dd6346469645545ad0773dd45555e35e78370113`，候选分支 `codex/public-v1`。
- 最新 [CI 34596889460](https://github.com/KG718718/spxt-public/actions/runs/34596889460) 成功：25份测试/734项（各套件报告计数）、失败0，依赖审计0漏洞。其中实际跨模块HTTP26项、完整角色浏览器21项、上传/OCR缺组件9项、核心包/恢复18项。
- 可选 OCR 的真实引擎另在 [CI 34577218357](https://github.com/KG718718/spxt-public/actions/runs/34577218357) 的 optional-ocr job 103192318451 通过6项；该旧run其他任务曾失败，不混写为整次成功。当前普通push跳过重装引擎，解析器与缺组件路径仍随每次运行。
- 已修复旧项目使用当前税率的3处页面读取、OCR项目归属门禁，以及审核按钮默认/悬停对比度；详见[本批证据](docs/tasks/public-v1/batch-15-workflow-and-core-package.md)。
- 云端核心ZIP只含32份允许的程序/许可/锁文件，附清单和SHA256；无业务/主数据、实例配置、测试、附件、Node或OCR/原生二进制。不是双击可用安装器。
- 解包后首Admin由使用者创建，其他业务表为空；同版本程序替换保持账号、主数据、配置与附件。停止服务后完整实例复制恢复到新隔离目录，通过字节及真实登录读取核对。不是任意旧版迁移或通用自动恢复入口。
- main保持 `7eb1ebe17450301aa0f290d55a5a0d3dd7ff2a6e`。公开产物仅在GitHub；未创建本地公开副本、任务或工作树，未改公司版/业务资产/正式服务，未使用真实SMTP，无PR/main合并/Release/部署。

### 当前需要用户选择，而非重复确认业务规则

离线完整包所需原生图形组件已核对项目级MIT，但二进制内全部第三方声明仍未核实齐全，尚未分发。已提出替代方案：首次安装联网，从官方来源取得锁定Node和依赖，之后可本机运行；该分发方式等待用户选择，不能因“loop”自行视为同意。

选择后继续最终安装器、干净安装及相应升级/恢复门禁。真正双机局域网仍需设备和网络方案授权，不使用生产主机替代；不承诺公网部署。

税率由Admin填写、不预置；MIT、购买方、替票授权、全部参数外显和既有修复均已确认，不再列为待确认。
...
完整server/Admin页面接入、其余业务页面、跨模块整站验收和可用安装包仍未完成；本次通过不等于全部封装完成。下一步已明确，无需重复询问税率或本次修复许可；不改公司版和19。此前“等待用户确认”等条目均为历史断点。
