...
本轮无本地公开仓库/源码/日志/产物落盘，只有内存中的文件转换、ZIP检验及 GitHub API。所有文件、安装目录、合成账号、截图与运行环境仅在云端。未读取或修改公司正式业务资产、19或真实SMTP；无 PR/main合并/Release/部署。
...
真实双机LAN、HTTPS/防火墙、系统服务、任意旧版本迁移、异机密钥恢复、离线整包、正式Release另定范围。安装失败缓存、旧版本与异常锁保留并在指南说明；不自动删除用户数据。当前门禁通过不等于所有未来输入无bug，也不授权在19上进行试装。
