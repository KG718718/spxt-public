...
默认 %LOCALAPPDATA%/K-SESSION/Beta/instance，日志位于该外置instance/logs/launcher.log（等价用户可写日志目录），临时文件instance/temp。不是Batch4最终安装升级路径。拒绝与程序目录重叠和重解析路径。--instance保留为测试/高级参数；普通用户双击无需配置。自动测试将LOCALAPPDATA定向全新E隔离目录，不写已有Beta数据。
