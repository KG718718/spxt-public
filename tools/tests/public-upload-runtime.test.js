...
(async()=>{try{
 ...
 await check('project response preserves attachment original name',async()=>{const data=await json('/api/data',owner);assert.equal(data.applications[0].attachments[0].originalName,'合成附件_中文.pdf');attachment=data.applications[0].attachments[0];});
