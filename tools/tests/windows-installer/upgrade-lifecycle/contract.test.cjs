...
test('real lifecycle has U01-U30 and all five required recoverable failure fixtures',()=>{
 ...
 const postInstall=iss.slice(iss.indexOf('procedure CurStepChanged'),iss.indexOf('function LockExecutable'));
 for(const marker of ['try','except','PostInstallFailed := True','KSESSION_UPGRADE_POSTINSTALL_FAILED','RaiseLastException','function GetCustomSetupExitCode','if PostInstallFailed then Result := 4'])assert.match(postInstall,new RegExp(marker.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
