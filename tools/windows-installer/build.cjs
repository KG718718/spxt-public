...
const info={product:'K⁺-SESSION Beta',qualification:mode==='candidate'?'UNSIGNED DEVELOPMENT ARTIFACT':
 (sequenceDiagnostic?'SEQUENCE DIAGNOSTIC - NEVER DISTRIBUTE OR INSTALL OUTSIDE DISPOSABLE HOST':'FAULT FIXTURE - NEVER DISTRIBUTE'),
...
if(sequenceDiagnostic)copyRuntime(path.join(__dirname,'sequence-identity.cjs'),'sequence-identity.cjs',[["require('./upgrade-detection/index.cjs')","require('./upgrade-detection.cjs')"]]);
