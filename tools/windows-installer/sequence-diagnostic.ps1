...
try{
  ...
  & $taskNode (Join-Path $PSScriptRoot 'build.cjs') $taskPortable $taskCompiler (Join-Path $taskBuild 'fault-payload-hash') $Commit sequence-payload-hash $taskBundle *> $null
  if($LASTEXITCODE -ne 0){throw 'SEQUENCE_PAYLOAD_HASH_BUILD_FAILED'}
