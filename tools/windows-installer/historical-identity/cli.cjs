...
const FOOTPRINT_EXIT = Object.freeze({MANIFEST: 30, BUILD_INFO: 31, BINDING: 32, RUNTIME: 33, LAUNCHER: 34,
  INVENTORY: 35, RUNTIME_HASH: 36, LAUNCHER_HASH: 37, PAYLOAD_COUNT: 38, PAYLOAD_PATH: 39,
  PAYLOAD_BYTES: 40, PAYLOAD_HASH: 41, PAYLOAD_SCHEMA: 42, USAGE: 43, OTHER: 44});
...
function installedFootprintCommand() {
  ...
  try { validateInstalledFootprint(args['--install-root'], args['--instance']); }
    ...
    const codes = {
      ...
      INSTALLED_PAYLOAD_PATH_CONFLICT: FOOTPRINT_EXIT.PAYLOAD_PATH,
